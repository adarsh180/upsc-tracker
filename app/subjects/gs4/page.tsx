...
export default function GS4SubjectsPage() {
  ...
  const fetchSubjects = async () => {
    try {
      ...
      // Filter GS4 subjects and ensure uniqueness
      const gs4Subjects = data.filter((s: SubjectProgress) => s.category === 'GS4');
      const uniqueSubjects = Object.values(
        gs4Subjects.reduce((acc: { [key: string]: SubjectProgress }, curr: SubjectProgress) => {
    ...
    <div className="min-h-screen p-6">
      ...
      >
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-4xl font-bold gradient-text">General Studies 4</h1>
            <p className="text-gray-300 mt-2">Ethics</p>
          </div>
        </div>
      </motion.div>
