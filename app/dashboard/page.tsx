...
// Lazy load all components for faster initial render
...
export default function Dashboard() {
  ...
  const loadDashboard = async () => {
    try {
      ...
      const isInitialized = sessionStorage.getItem('db_initialized');

      if (!isInitialized) {
        // Run init calls in parallel (not sequential)
        await Promise.all([
          fetch('/api/init', { method: 'POST' }),
          fetch('/api/subjects/init', { method: 'POST' })
        ]);
